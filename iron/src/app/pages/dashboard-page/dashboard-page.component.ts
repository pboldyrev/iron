...
import {
  AssetTableColumn,
  AssetTableComponent,
} from './asset-table/asset-table.component';
...
import {
  ASSET_TABLE_COLS_COLLAPSED,
  ASSET_TABLE_COLS_EXPANDED,
  ASSET_TABLE_FOOTER_COLS,
...
export class DashboardPageComponent implements AfterContentInit {
  ...
  public isNetworthLoading$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  public isAssetsLoading$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);

  public totalNetworth: number = 0;
  public networthTimeframes: ValueChange[] = [];
  public networthValues$: BehaviorSubject<AssetValue[]> = new BehaviorSubject<
    AssetValue[]
  >([]);
  ...
  showSidebar =
    (this.preferencesService.getPreference(USER_PREFERENCES.ShowSidebar) ??
      'true') === 'true';
  ...
  ngAfterContentInit() {
    this.networthValues$.pipe(skip(1)).subscribe((data: AssetValue[]) => {
      if (this.dashboardChart) {
        this.dashboardChart.data = this.chartService.getDataSet(data);
        this.dashboardChart.options.borderColor =
  ...
  private fetchAssets(): void {
    this.dataService
      .getAssets$(this.isAssetsLoading$)
      .pipe(
        takeUntilDestroyed(),
        tap((assets: Asset[]) => {
  ...
  private fetchNetWorth(): void {
    this.dataService
      .getNetWorthValues$(null, this.isNetworthLoading$)
      .pipe(
        takeUntilDestroyed(),
  ...
  private setValueChanges(networthValues: AssetValue[]): void {
    ...
    const sinceLastChange = this.getValueChange(
      ...
      'Since ' +
        new Date(
          networthValues[networthValues.length - 2].timestamp ?? 0,
