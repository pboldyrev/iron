export const TEXTS = {
  TITLE: '👋 Log in',
  SUBTITLE: 'Use your email to authenticate into Finacle.',
  INVALID_CREDENTIALS: 'Invalid credentials.',
  UNKNWON_LOGIN_ERROR: 'Something went wrong. Please try again!',
  INCORRECT_CODE: 'This one time code is incorrect.',
  HEADLINE:
    'Finacle is a privacy-first asset tracking tool, supercharged by AI.',
  SUBHEADLINE:
    'Get a clear view into your past, present, and future net worth with the same level of privacy as a spreadsheet. You can try it out for free!',
};

export const LABELS = {
  EMAIL: 'Email',
  BETA: 'Invite code',
  OTP: 'One time code',
};

export const TOOLTIPS = {
  BETA: 'Finacle is currently in Alpha. We are only accepting a small number of users. Please email hello@finacle.app if you would like access.',
  EMAIL: '',
  OTP: 'This is a 6 digit numerical code sent to the provided email.',
};

export const BTN_TEXTS = {
  LOGIN: 'Get verification code',
  SUBMIT_CODE: 'Submit code',
  BACK: 'Back',
};
