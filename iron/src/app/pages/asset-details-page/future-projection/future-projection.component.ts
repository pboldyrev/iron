...
export class FutureProjectionComponent {
  ...
  updateChart(): void {
    ...
    if (this.projectionChart) {
      ...
      this.projectionChart.options.borderColor =
        this.chartService.getBorderColor(projections);
      this.projectionChart.update();
    } else {
      this.projectionChart = new Chart(
        'projectionChart',
        this.chartService.getOptions(projections),
      );
    }
  }

  private getValueProjections(
    ...
    this.valueProjection = latestValue
      .toFixed(2)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    return projection;
